...
public class Day5 {
    ...
    private Map<Long, Long> convertCategory(String string) {
        String [] lines = string.split("\n");
        Map<Long, Long> categoryMap = new HashMap<>();
        for (String line : Arrays.stream(lines).filter(line -> !numbers(line).isEmpty()).toList()) {
            List<Long> numbers = numbers(line);
            for (int i = 0; i < numbers.get(2); i++) {
                categoryMap.put(numbers.get(1) + i, numbers.get(0) + i);
            }
        }
        return categoryMap;
    }
    private long lowestLocation(String string) {
        List<String> categories = new java.util.ArrayList<>(Arrays.stream(string.split(":")).toList());
        for (int i = 1; i < categories.size(); i++) {
            String category = categories.get(i);
            StringBuilder stringBuilder = new StringBuilder();
            for (char c : category.toCharArray()) {
                if (!stringBuilder.isEmpty() && c == '\n' && stringBuilder.charAt(stringBuilder.length() - 1) != '\n') stringBuilder.append(c);
                if (Character.isDigit(c) || Character.isSpaceChar(c)) stringBuilder.append(c);
            }
            categories.set(i, stringBuilder.toString());
        }
        categories.remove(0);
        //go through the categories
        Map<Long, Long []> seed = new HashMap<>();
        for (Long number : numbers(categories.get(0))) {
            Long [] array = new Long[8];
            array[0] = number;
            seed.put(number, array);
        }
        int categoryNumber = 1;
        for (String category : categories.subList(1, categories.size())) {
            Map<Long, Long> mapCategory = convertCategory(category);
            for (Long seedNumber : seed.keySet()) {
                Long [] array = seed.get(seedNumber);
                array[categoryNumber] = mapCategory.containsKey(array[categoryNumber - 1]) ? mapCategory.get(array[categoryNumber - 1]) : array[categoryNumber - 1];
                seed.put(seedNumber, array);
            }
            categoryNumber++;
        }
        //find lowest
        long ret = - 1;
        for (Long [] array : seed.values()) {
            if (array[array.length - 1] < ret || ret == - 1) {
